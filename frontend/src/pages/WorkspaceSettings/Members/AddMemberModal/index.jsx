...
export default function AddMemberModal({ closeModal, workspace, users }) {
  ...
  const handleUpdate = async (e) => {
    ...
    if (success) {
      showToast(t("workspace_members.modal.updated_successfully"), "success");
      setTimeout(() => {
      ...
      <div className="w-full max-w-2xl bg-theme-bg-secondary rounded-lg shadow border-2 border-theme-modal-border overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b rounded-t border-theme-modal-border">
          <div className="flex items-center gap-x-4">
            <h3 className="text-base font-semibold text-white">
              {t("workspace_members.modal.title")}
            </h3>
            <div className="relative">
              <input
                ...
                placeholder={t("workspace_members.modal.search_placeholder")}
              />
              <MagnifyingGlass
                size={16}
                weight="bold"
        ...
        <form onSubmit={handleUpdate}>
            ...
            <table className="gap-y-[8px] flex flex-col max-h-[385px] overflow-y-auto no-scroll">
              {filteredUsers.length > 0 ? (
                ...
                <p className="text-theme-text-secondary text-sm font-medium ">
                  {t("workspace_members.modal.no_users")}
                </p>
              )}
            </table>
          </div>
          <div className="flex w-full justify-between items-center p-3 space-x-2 border-t rounded-b border-gray-500/50">
            <div className="flex items-center gap-x-2">
              ...
              >
                ...
                <p className="text-white text-sm font-medium">
                  {t("workspace_members.modal.select_all")}
                </p>
              </button>
              {selectedUsers.length > 0 && (
                ...
                >
                  <p className="text-theme-text-secondary text-sm font-medium hover:text-theme-text-primary">
                    {t("workspace_members.modal.unselect")}
                  </p>
                </button>
              )}
            </div>
            <button
              type="submit"
              className="ui-btn-primary transition-all duration-300 text-xs px-2 py-1 font-semibold rounded-lg bg-primary-button border-2 border-transparent h-[32px] w-[68px] -mr-8 whitespace-nowrap shadow-[0_4px_14px_rgba(0,0,0,0.25)]"
            >
              {t("workspace_members.modal.save")}
            </button>
          </div>
        </form>
      </div>
