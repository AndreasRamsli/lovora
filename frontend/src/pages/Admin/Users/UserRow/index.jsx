...
export default function UserRow({ currUser, user }) {
  ...
  const handleSuspend = async () => {
    if (
      !window.confirm(
        t("users.row.suspend_confirm", { username: user.username })
      )
    )
      return false;
    ...
    if (success) {
      showToast(
        !suspended ? t("users.row.suspended") : t("users.row.unsuspended"),
        "success",
        { clear: true }
      );
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        t("users.row.delete_confirm", { username: user.username })
      )
    )
      return false;
    ...
    if (success) {
      ...
      showToast(t("users.row.deleted"), "success", { clear: true });
    }
  };
        ...
        <td className="px-6 flex items-center gap-x-6 h-full mt-2">
          ...
          {currUser?.id !== user.id && canModify && (
            <>
              ...
              >
                {suspended ? t("users.row.unsuspend") : t("users.row.suspend")}
              </button>
              <button
