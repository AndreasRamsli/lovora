    ...
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-theme-text-primary mb-2">
          {t("agent_builder.api_call.url")}
        </label>
        <div className="flex gap-2">
          <input
            ref={urlInputRef}
            type="text"
            placeholder={t("agent_builder.api_call.url_placeholder")}
            value={config.url}
          ...
          <div className="relative">
            <button
              ...
              title={t("agent_builder.api_call.insert_variable")}
            >
            ...
            {showVarMenu && (
              <div className="absolute right-0 top-[calc(100%+4px)] w-48 bg-theme-settings-input-bg border-none rounded-lg shadow-lg z-10">
                {renderVariableSelect(
                  ...
                  t("agent_builder.api_call.select_variable_to_insert"),
                  true
                )}
              </div>
            )}
      ...
      <div>
        <label className="block text-sm font-medium text-theme-text-primary mb-2">
          {t("agent_builder.api_call.method")}
        </label>
        <select
          value={config.method}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-sm font-medium text-theme-text-primary">
            {t("agent_builder.api_call.headers")}
          </label>
          <button
            onClick={addHeader}
            ...
            title={t("agent_builder.api_call.add_header")}
          >
        ...
        <div className="space-y-2">
          {(config.headers || []).map((header, index) => (
            <div key={index} className="flex gap-2">
              <input
                type="text"
                placeholder={t("agent_builder.api_call.header_name")}
                value={header.key}
                onChange={(e) =>
              ...
              <input
                type="text"
                placeholder={t("agent_builder.api_call.value")}
                value={header.value}
                onChange={(e) =>
              ...
              <button
                ...
                title={t("agent_builder.api_call.remove_header")}
              >
                <X className="w-4 h-4" />
              </button>
            </div>
      ...
      {["POST", "PUT", "PATCH"].includes(config.method) && (
        <div>
          <label className="block text-sm font-medium text-theme-text-primary mb-2">
            {t("agent_builder.api_call.request_body")}
          </label>
          <div className="space-y-2">
            <select
            ...
            >
              ...
              <option
                ...
                className="bg-theme-bg-primary light:bg-theme-settings-input-bg"
              >
                {t("agent_builder.api_call.raw_text")}
              </option>
              <option
                value="form"
                className="bg-theme-bg-primary light:bg-theme-settings-input-bg"
              >
                {t("agent_builder.api_call.form_data")}
              </option>
            </select>
              ...
              <div className="space-y-2">
                {(config.formData || []).map((item, index) => (
                  <div key={index} className="flex gap-2">
                    <input
                      type="text"
                      placeholder={t("agent_builder.api_call.key")}
                      value={item.key}
                      onChange={(e) => {
                    ...
                    <button
                      ...
                      title={t("agent_builder.api_call.remove_field")}
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                ...
                >
                  {t("agent_builder.api_call.add_field")}
                </button>
              </div>
            ) : (
              <textarea
                placeholder={t("agent_builder.api_call.raw_body_placeholder")}
                value={config.body}
                onChange={(e) => onConfigChange({ body: e.target.value })}
      ...
      <div>
        <label className="block text-sm font-medium text-theme-text-primary mb-2">
          {t("agent_builder.api_call.response_variable")}
        </label>
        {renderVariableSelect(
