        ...
        <div className="relative p-6 border-b rounded-t border-theme-modal-border">
          <div className="w-full flex gap-x-2 items-center">
            <h3 className="text-xl font-semibold text-white overflow-hidden overflow-ellipsis whitespace-nowrap">
              {t("browser_extension_api_keys.modal.title")}
            </h3>
          </div>
          <button
            onClick={closeModal}
            ...
            <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2">
              {error && (
                <p className="text-red-400 text-sm">
                  {t("browser_extension_api_keys.modal.error", { error })}
                </p>
              )}
              {apiKey && (
                <input
              ...
              {isMultiUser && (
                <p className="text-yellow-300 light:text-orange-500 text-xs md:text-sm font-semibold">
                  {t("browser_extension_api_keys.modal.multi_user_warning")}
                </p>
              )}
              <p className="text-white text-opacity-60 text-xs md:text-sm">
                {t("browser_extension_api_keys.modal.auto_connect_description")}
              </p>
              <p className="text-white text-opacity-60 text-xs md:text-sm">
                {t("browser_extension_api_keys.modal.success_description")}
              </p>
            </div>
            <div className="flex justify-between items-center mt-6 pt-6 border-t border-theme-modal-border">
              {!apiKey ? (
                <>
                  ...
                  >
                    {t("browser_extension_api_keys.modal.cancel")}
                  </button>
                  <button
                    type="submit"
                    className="transition-all duration-300 bg-white text-black hover:opacity-60 px-4 py-2 rounded-lg text-sm"
                  >
                    {t("browser_extension_api_keys.modal.create")}
                  </button>
                </>
              ) : (
                <button
                ...
                >
                  {copied
                    ? t("browser_extension_api_keys.modal.copied")
                    : t("browser_extension_api_keys.modal.copy")}
                </button>
              )}
            </div>
          </form>
