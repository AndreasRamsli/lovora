...
export default function DeviceRow({ device, removeDevice }) {
  ...
  const handleApprove = async () => {
    ...
    showToast(t("mobile_connections.row.granted"), "info");
    setStatus(true);
  };

  const handleDeny = async () => {
    await MobileConnection.deleteDevice(device.id);
    showToast(t("mobile_connections.row.denied"), "info");
    setStatus(false);
      ...
      <tr className="bg-transparent text-white text-opacity-80 text-xs font-medium border-b border-white/10 h-10">
        ...
        <td className="px-6 flex items-center gap-x-6 h-full mt-1">
          {status ? (
            ...
            >
              {t("mobile_connections.row.revoke")}
            </button>
          ) : (
            <>
              <button
              ...
              >
                {t("mobile_connections.row.approve")}
              </button>
              <button
                onClick={handleDeny}
