...
export default function SystemPrompt({ item, setStep }) {
  ...
  async function handleSubmit() {
    showToast(t("community_hub.import.item.system_prompt.applying"), "info");
    const { error } = await CommunityHub.applyItem(item.importId, {
      workspaceSlug: destinationWorkspaceSlug,
    });
    if (error) {
      return showToast(
        t("community_hub.import.item.system_prompt.apply_error", {
          error,
        }),
        "error",
        {
    ...
    showToast(
      t("community_hub.import.item.system_prompt.apply_success"),
      "success",
      {
        clear: true,
      }
    ...
    <div className="flex flex-col mt-4 gap-y-4">
      <div className="flex flex-col gap-y-1">
        <h2 className="text-base text-theme-text-primary font-semibold">
          {t("community_hub.import.item.system_prompt.review_title", {
            name: item.name,
          })}
        </h2>
      ...
      <div className="flex flex-col gap-y-[25px] text-white/80 light:text-theme-text-secondary text-sm">
        <p>{t("community_hub.import.item.system_prompt.description")}</p>

        <div className="flex flex-col gap-y-2">
          <p className="text-white/60 light:text-theme-text-secondary font-semibold">
            {t("community_hub.import.item.system_prompt.provided_prompt")}
          </p>
        ...
        <div className="flex flex-col w-60">
          <label className="text-theme-text-primary text-sm font-semibold block mb-3">
            {t("community_hub.import.item.system_prompt.apply_to_workspace")}
          </label>
          <select
          ...
          >
            <optgroup
              label={t(
                "community_hub.import.item.system_prompt.available_workspaces"
              )}
            >
      ...
      {destinationWorkspaceSlug && (
        ...
        >
          {t("community_hub.import.item.system_prompt.apply_button")}
        </CTAButton>
      )}
    </div>
  );
