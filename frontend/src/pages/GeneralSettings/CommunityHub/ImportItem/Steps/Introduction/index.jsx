...
export default function Introduction({ settings, setSettings, setStep }) {
  ...
  const handleContinue = () => {
    if (!itemId)
      return showToast(
        t("community_hub.import.introduction.missing_id"),
        "error"
      );
      ...
      <div className="bg-theme-bg-secondary rounded-xl flex-1 p-6">
        <div className="w-full flex flex-col gap-y-2 max-w-[700px]">
          <h2 className="text-base text-theme-text-primary font-semibold">
            {t("community_hub.import.introduction.title")}
          </h2>
          <div className="flex flex-col gap-y-[25px] text-theme-text-secondary text-sm">
            <p>{t("community_hub.import.introduction.description_1")}</p>
            <p>{t("community_hub.import.introduction.description_2")}</p>
            <p>
              <Trans
                i18nKey="community_hub.import.introduction.description_3"
                components={{ bold: <b /> }}
              />
            </p>
            ...
            <p className="p-4 bg-yellow-800/30 light:bg-orange-100 light:text-orange-500 light:border-orange-500 rounded-lg border border-yellow-500 text-yellow-500">
              <Trans
                i18nKey="community_hub.import.introduction.warning"
                components={{
                  bold: <b />,
                  link: (
                    <a
            ...
            <div className="w-full flex flex-col gap-y-4">
              <div className="flex flex-col w-full">
                <label className="text-theme-text-primary text-sm font-semibold block mb-3">
                  {t("community_hub.import.introduction.item_id_label")}
                </label>
                <input
                  type="text"
                  value={itemId}
                  ...
                  placeholder={t(
                    "community_hub.import.introduction.item_id_placeholder"
                  )}
          ...
          >
            {t("community_hub.import.introduction.continue")}
          </CTAButton>
        </div>
      </div>
    </div>
