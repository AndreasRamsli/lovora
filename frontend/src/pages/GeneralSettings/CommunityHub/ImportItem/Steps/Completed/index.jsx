      ...
      <div className="bg-theme-bg-secondary rounded-xl flex-1 p-6">
        <div className="w-full flex flex-col gap-y-2 max-w-[700px]">
          <h2 className="text-base text-theme-text-primary font-semibold">
            {t("community_hub.import.completed.title")}
          </h2>
          <div className="flex flex-col gap-y-[25px] text-theme-text-secondary text-sm">
            <p>
              {t("community_hub.import.completed.success", {
                name: settings.item.name,
                itemType: settings.item.itemType,
              })}
            </p>
            {settings.item.itemType === "agent-flow" && (
              ...
              >
                {t("community_hub.import.completed.view_agent_skills", {
                  name: settings.item.name,
                })}
              </Link>
            )}
            <p>
              {t("community_hub.import.completed.changes_note", {
                itemType: settings.item.itemType,
              })}
            </p>
          </div>
          ...
          >
            {t("community_hub.import.completed.import_another")}
          </CTAButton>
        </div>
      </div>
    </div>
