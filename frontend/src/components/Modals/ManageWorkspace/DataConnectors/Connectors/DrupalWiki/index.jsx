...
export default function DrupalWikiOptions() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      showToast(t("connectors.drupalwiki.fetching"), "info", {
        clear: true,
        autoClose: false,
      });
      ...
      showToast(
        t("connectors.drupalwiki.success", {
      ...
      <div className="flex flex-col w-full px-1 md:pb-6 pb-16">
        <form className="w-full" onSubmit={handleSubmit}>
            ...
            <div className="w-full flex flex-col gap-4">
              <div className="flex flex-col pr-10">
                <div className="flex flex-col gap-y-1 mb-4">
                  ...
                  <p className="text-xs font-normal text-theme-text-secondary">
                    <Trans
                      i18nKey="connectors.drupalwiki.base_url_help"
                      components={{
                        link: (
                          <a
                ...
                <input
                  ...
                  placeholder={t("connectors.drupalwiki.base_url_placeholder")}
                  required={true}
                  autoComplete="off"
                  spellCheck={false}
                />
              ...
              <div className="flex flex-col pr-10">
                <div className="flex flex-col gap-y-1 mb-4">
                  <label className="text-white text-sm font-bold">
                    {t("connectors.drupalwiki.space_ids")}
                  </label>
                  <p className="text-xs font-normal text-theme-text-secondary">
                    <Trans
                      i18nKey="connectors.drupalwiki.space_ids_help"
                      components={{
                        manual: (
                          <a
                ...
                <input
                  ...
                  name="spaceIds"
                  className="border-none bg-theme-settings-input-bg text-white placeholder:text-theme-settings-input-placeholder text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5"
                  placeholder={t("connectors.drupalwiki.space_ids_placeholder")}
                  required={true}
                  autoComplete="off"
                  spellCheck={false}
                />
              ...
              <div className="flex flex-col pr-10">
                <div className="flex flex-col gap-y-1 mb-4">
                  <label className="text-white text-sm font-bold flex gap-x-2 items-center">
                    <p className="font-bold text-white">
                      {t("connectors.drupalwiki.api_token")}
                    </p>
                    <Warning
                      size={14}
                    ...
                    >
                      <p className="text-sm font-light text-theme-text-primary">
                        <Trans
                          i18nKey="connectors.drupalwiki.api_token_help"
                          components={{
                            manual: (
                              <a
                  ...
                  <p className="text-xs font-normal text-theme-text-secondary">
                    {t("connectors.drupalwiki.api_token_description")}
                  </p>
                </div>
                <input
                  type="password"
          ...
          <div className="flex flex-col gap-y-2 w-full pr-10">
            ...
            >
              {loading
                ? t("connectors.drupalwiki.collecting")
                : t("connectors.drupalwiki.submit")}
            </button>
            {loading && (
              <p className="text-xs text-theme-text-secondary">
                {t("connectors.drupalwiki.loading_note")}
              </p>
            )}
          </div>
        </form>
