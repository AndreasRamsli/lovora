...
function ActionMenu({ chatId, forkThread, isEditing, role }) {
  ...
  return (
    <div className="mt-2 -ml-0.5 relative" ref={menuRef}>
      <button
        onClick={toggleMenu}
        className="border-none text-theme-text-secondary hover:text-theme-text-primary transition-colors duration-200"
      ...
      {open && (
        <div className="absolute -top-1 left-7 mt-1 border border-theme-modal-border rounded-lg bg-theme-action-menu-bg flex flex-col shadow-[0_4px_14px_rgba(0,0,0,0.25)] text-theme-text-primary z-99 md:z-10">
          <button
            onClick={handleFork}
